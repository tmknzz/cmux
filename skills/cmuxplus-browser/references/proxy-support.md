...
How proxy behavior works for cmuxplus browser automation.
...
cmuxplus browser uses WKWebView networking. Proxy behavior follows macOS/system networking and app process environment.

## What Is Not Exposed via CLI

There is currently no first-class `cmuxplus browser proxy ...` command for per-surface proxy routing.
...
```bash
cmuxplus browser open https://httpbin.org/ip --json
