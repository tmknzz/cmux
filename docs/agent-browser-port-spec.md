...
Primary form:
```bash
cmuxplus browser --surface <surface-id> <agent-browser-style-command...>
```

Shorthand:
```bash
cmuxplus browser <surface-id> <agent-browser-style-command...>
```

Agent discovery:
```bash
cmuxplus identify
cmuxplus capabilities
cmuxplus browser identify --surface <surface-id>   # wrapper over system.identify + browser fields
```

Flash:
```bash
cmuxplus trigger-flash [--workspace <id>] [--surface <id>]
```
...
1. `cmuxplus browser tab ...` maps to browser `surface` tabs only (no separate workspace-level tab meaning inside `browser` namespace).
...
2. Whether to expose protocol-only agent-browser actions in first public release of `cmuxplus browser` or gate them behind a second rollout phase.
