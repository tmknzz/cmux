...
APP="$DERIVED_DATA_PATH/Build/Products/Debug/cmuxplus DEV.app"
RUN_TAG="tests-v2"
...
if [ ! -d "$APP" ]; then
  echo "ERROR: cmuxplus DEV.app not found at expected path: $APP" >&2
  exit 1
fi

cleanup() {
  pkill -x "cmuxplus DEV" || true
  pkill -x "cmuxplus" || true
...
launch_and_wait() {
  ...
  for _ in {1..50}; do
    pgrep -x "cmuxplus DEV" >/dev/null 2>&1 || break
    sleep 0.1
  done

  # Force socket mode for deterministic automation runs, independent of prior user settings.
  defaults write com.cmuxplus.app.debug socketControlMode -string full >/dev/null 2>&1 || true

  # Launch directly with UI test mode enabled so startup follows deterministic test codepaths.
  CMUX_TAG="$RUN_TAG" CMUX_UI_TEST_MODE=1 "$APP/Contents/MacOS/cmuxplus DEV" >/dev/null 2>&1 &
