...
if ! grep -Fq 'com.cmuxplus.app.nightly.universal' "$WORKFLOW_FILE"; then
  echo "FAIL: nightly workflow must set a distinct .universal bundle ID"
  exit 1
fi

if ! grep -Fq 'https://github.com/tamekuniz/CMUX-Plus/releases/download/nightly/appcast-universal.xml' "$WORKFLOW_FILE"; then
...
if ! awk '
  ...
  in_upload && /cmuxplus-nightly-macos\*\.dmg/ { saw_arm_artifacts=1 }
  in_upload && /cmuxplus-nightly-universal-macos\*\.dmg/ { saw_universal_artifacts=1 }
